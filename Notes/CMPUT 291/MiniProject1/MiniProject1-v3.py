...
'''
    ...
    if len(postList)<5:
        ...
        endBoolean=False
        while endBoolean!=True:
            for i in range(c,d):
                print(postList[i])
            selected=input("Would you like to select a post? Input Y/N ")
            while selected not in "YyNn":
                selected=input("Incorrect Choice. Would you like to select a post? Input Y/N ")
            if selected in "Nn":
                if len(postList)-i<5:
                    for i in range(i,len(postList)):
                        print(postList[i])
                    endBoolean=True
                else:
                    c=i
