...
def get_uid(): # get the uid from the user
    ...
    while valid == False:#Checks Validity of User ID, continues to loop until it is valid
        uid = input('Please enter a User ID (Press enter to continue without using a User ID): ')
        if uid == '':
            valid = True
        elif uid == 'e':
...
def norm_menu(db, uid): # first menu when getting inside the program
    valid = False
    while valid == False:#Checks if option is valid
...
def post_a_question(db, uid):
    ...
    cho_opt = input('If you want to go back to the main page, please enter "q". Press enter any key to continue. ')
    if cho_opt.lower().strip() == 'q':
        ...
        tag_list = []
        fin = False
        while fin == False:#Loops till the user manually stops entering tags by pressing enter
            keyword = input('Enter a tag. (Press enter to continue without entering a tag): ')
            if keyword == "":
                fin = True
            else:
                in_fin = False
...
def search_for_questions(db, uid):
    ...
    fin = False
    while fin == False:
        keyword = input('Enter a keyword. Press enter to not enter a keyword: ')
        if keyword == "":
            fin = True
        else:
            in_fin = False
            while in_fin == False:#Loops till the user decides to not enter anymore keywords
    ...
    print("-"*90)
    for key in key_list:
        find = db.Posts.find({"PostTypeId": "1"})#Gets all posts with post type ID of 1
        for posts in find:
            title = posts["Title"]
            body = posts["Body"]
    ...
    while valid == False:
        ...
        chosen = input('\nEnter the Post ID of the post you wish to choose or enter "q" to go to the main page: ')
        if chosen.strip() == 'q':
            valid = True
            norm_menu(db, uid)
            ...
            print('View Count: {}'.format(view_count))
...
def question_action_answer(db, uid, pid):
    ...
    if post_type == "1": # input information
        ...
        print('This is not a question post.')
        norm_menu(db, uid)
...
def action_vote(db, uid, pid):
    ...
    if uid != "-1": # Checks if user is anonymous
        ...
        if with_user == []: # insert the vote with the votes that does not contain user id
            ...
            print("Successfully voted on PID {}, the Vote ID is {}.".format(pid, vid))
            
        elif with_user != []:
            all_used_uid = []
            for i in with_user:
                all_used_uid.append(i["UserId"])
            if uid in all_used_uid:
    ...
    db.Posts.update_one({"Id": pid},{"$set": {"Score": new_score}}) # After incrementing by one, updates db
    norm_menu(db, uid)

def report(db, uid):
    # check if the user id appears in the database
    print('Searching for User Report...')
    has_id = []
    find = db.Posts.find()
    for i in find:
        if "OwnerUserId" in i.keys():
            has_id.append(i)
    owner = [] # this is the list that contains uid with the OwnerUserId 
    for i in has_id:
        owner.append(i["OwnerUserId"])
    
    if uid not in owner:
        print("\n-- Report Not Found --")
        print("Logging in as User ID {} to the main menu...".format(uid))
        norm_menu(db, uid)
    
    # find how many questions and answers
    find = db.Posts.find({"OwnerUserId": uid})
    question_own = []
    answer_own = []
    for i in find:
        if i["PostTypeId"] == "1":
            question_own.append(i)
        elif i["PostTypeId"] == "2":
            answer_own.append(i)
    ...
    print("\n-- User Report --")
    print("The number of QUESTION owned:", str(len(question_own)))
    print("The average score for the QUESTIONS:", avg_q)
    print("The number of ANSWER owned:", str(len(answer_own)))
