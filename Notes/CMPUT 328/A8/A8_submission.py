...
#UNet model from here: https://github.com/milesial/Pytorch-UNet
...
def segmentate(images):
    ...
    for i in range(N):
        img_ndarray = np.asarray(images[i])
        img_ndarray = (img_ndarray.transpose((2, 0, 1)))/255
        curr_img= (torch.from_numpy(img_ndarray)).unsqueeze(0).to(device='cuda', dtype=torch.float32)
        with torch.no_grad():
            output = model(curr_img)
            full_mask = (F.softmax(output, dim=1)[0]).cpu().squeeze()
        mask=F.one_hot(full_mask.argmax(dim=0), model.n_classes).permute(2, 0, 1).numpy()
        pred_seg[i]=np.argmax(mask, axis=0).flatten()
